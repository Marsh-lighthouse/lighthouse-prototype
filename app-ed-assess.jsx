        ...
        { ic: "lock", t: "Confidential", d: "Responses visible only to authorized assessors" }].map((it, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 13, padding: "15px 17px", background: eCARD, borderRadius: 14, border: "1px solid " + eLINE }}>
              ...
              <div><div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>{it.t}</div><div style={{ fontFamily: "var(--sans)", fontSize: 15, color: eINK, lineHeight: 1.5 }}>{it.d}</div></div>
            </div>);
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,15,71,.45)", backdropFilter: "blur(3px)", WebkitBackdropFilter: "blur(3px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 18, width: "min(820px, 100%)", maxHeight: "88vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 24px 70px rgba(0,15,71,.3)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 22px", borderBottom: "1px solid " + eLINE, flexShrink: 0 }}>
          <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, letterSpacing: 0.4, color: eBLUE }}>MARSH · CONFIDENTIAL</span>
        ...
        <div style={{ overflowY: "auto", padding: "28px 32px" }}>
          ...
          <div style={{ border: "1px solid " + eLINE, borderRadius: 12, overflow: "hidden", marginTop: 4 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", background: eCARD, borderBottom: "1px solid " + eLINE }}>
              <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, padding: "12px 16px" }}>Category</div>
              <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, padding: "12px 16px", borderLeft: "1px solid " + eLINE }}>Examples</div>
            </div>
            {rows.map((r, i) =>
...
function EdConsent({ exercise, onAccept, onExit }) {
  ...
  return (
    <div style={{ maxWidth: "var(--content-max)", margin: "36px var(--fol-mx) 72px", padding: 0 }}>
      <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, letterSpacing: 0.4, color: eBLUE, marginBottom: 12 }}>MARSH · CONFIDENTIAL</div>
    ...
    <div style={{ background: "var(--card)", border: "1px solid " + (error ? eDANGER : eLINE), borderRadius: 16, padding: "26px 28px", transition: "border-color .15s" }}>
                ...
                {q.cols.map((col, ci) => {
                  ...
                  return (
                    <button key={ci} className="oa-mopt" onClick={() => onChange({ ...a, [ri]: ci })} style={{ flex: "1 1 auto", minWidth: 72, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "9px 12px", borderRadius: 9, border: "1.5px solid " + (sel ? eBLUE : eLINE), background: sel ? "color-mix(in srgb, var(--accent) 6%, transparent)" : "#fff", cursor: "pointer", transition: "all .15s" }}>
                      ...
                      <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: sel ? 700 : 700, color: sel ? eMID : eINK }}>{col}</span>
                    </button>);
                })}
              </div>
            </div>);
                    ...
                    {cols.map((col, ci) => {
                      ...
                      return (
                        <button key={ci} onClick={() => toggle(ri, ci)} aria-label={row + " — " + col} style={{ width: "100%", display: "inline-flex", alignItems: "center", justifyContent: "flex-start", gap: 10, padding: "10px 14px", borderRadius: 9, border: "1.5px solid " + (on ? eMID : eLINE), background: on ? "var(--track)" : "#fff", cursor: "pointer", textAlign: "left", transition: "all .15s" }}>
                          ...
                          <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: on ? 700 : 700, color: on ? eMID : eINK }}>{col}</span>
                        </button>);
                    })}
                  </div>
                </div>
            ...
            return (
              <div>
                <div style={{ display: "grid", gridTemplateColumns: gridCols, columnGap: 14, padding: "0 4px 10px", borderBottom: "1px solid " + eLINE, alignItems: "end" }}>
                  <div />
                  {cols.map((col, ci) => <div key={ci} style={{ textAlign: "center", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, lineHeight: 1.25, padding: "0 6px" }}>{col}</div>)}
                </div>
                <div>
                  {q.rows.map((row, ri) => {
                    const cur = Array.isArray(v[ri]) ? v[ri] : [];
                    return (
                      <div key={ri} style={{ display: "grid", gridTemplateColumns: gridCols, columnGap: 14, alignItems: "center", padding: "14px 4px", borderBottom: "1px solid " + eLINE }}>
                        <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, lineHeight: 1.35, paddingRight: 10 }}>{row}</div>
                        {cols.map((col, ci) => {
                          const on = cur.includes(ci);
                          return (
                            <div key={ci} style={{ display: "flex", justifyContent: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              return (
                <div key={ri} style={{ padding: "14px 16px", borderRadius: 12, background: eCARD, border: "1px solid " + eLINE }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
                    <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, lineHeight: 1.4 }}>{row}</span>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "stretch" }}>
                    {q.cols.map((col, ci) => (
                      <label key={ci} style={{ flex: "0 0 auto", display: "flex", flexDirection: "column", gap: 5 }}>
                        <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMUT }}>{col}</span>
                    ...
                    <button onClick={() => setRow(ri, { na: !na })} aria-label={row + " — Not Applicable"} style={{ flex: "0 0 auto", height: 40, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "0 12px", borderRadius: 8, border: "1.5px solid " + (na ? eMID : eLINE), background: na ? "var(--track)" : "#fff", cursor: "pointer", whiteSpace: "nowrap" }}>
                      ...
                      <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, whiteSpace: "nowrap" }}>Not Applicable</span>
                    </button>
                  </div>
                </div>
              );
            ...
            return (
              <div>
                <div style={{ display: "grid", gridTemplateColumns: gridCols, columnGap: 14, padding: "0 4px 10px", borderBottom: "1px solid " + eLINE, alignItems: "end" }}>
                  <div />
                  {q.cols.map((col, ci) => <div key={ci} style={{ textAlign: "center", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, lineHeight: 1.25, padding: "0 4px" }}>{col}</div>)}
                  <div style={{ textAlign: "center", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>Not Applicable</div>
                  <div style={{ textAlign: "center", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>Total</div>
                </div>
                <div>
                  {q.rows.map((row, ri) => {
                    const r = v[ri] || {};
              ...
              <div>
                {q.labels &&
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    {q.labels.map((l, li) => <div key={li} style={{ flex: 1, textAlign: li === 0 ? "left" : li === q.labels.length - 1 ? "right" : "center", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>{l}</div>)}
                  </div>}
              </div>
              <div />
            </div>
              ...
              {q.rows.map((row, ri) => {
                ...
                return (
                  <div key={ri} style={{ display: "grid", gridTemplateColumns: "minmax(120px,1.1fr) 3fr 64px", gap: 14, alignItems: "center" }}>
                    <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>{row}</div>
              ...
              <div>
                {q.labels &&
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    {q.labels.map((l, li) => <div key={li} style={{ flex: 1, textAlign: "center", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>{l}</div>)}
                  </div>}
              ...
              {q.rows.map((row, ri) => {
                ...
                return (
                  <div key={ri} style={{ display: "grid", gridTemplateColumns: "minmax(120px,1fr) 3fr 44px", gap: 12, alignItems: "center", padding: "10px 0", borderBottom: "1px solid " + eLINE }}>
                    <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, paddingLeft: 10 }}>{row}</div>
                    ...
                    <div style={{ textAlign: "right", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: val == null ? eMUT : eMID }}>{val == null ? "–" : val}</div>
                  </div>
                );
              })}
            </div>
      ...
      {q.type === "fillgauge" && (() => {
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 22, padding: "6px 0" }}>
            ...
            <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>{filled} / {segs}</div>
          </div>
        );
      })()}
      ...
      {q.type === "audio" && (value ?
        ...
        <div style={{ textAlign: "center", padding: 22, background: eCARD, border: "1px solid " + eLINE, borderRadius: 14 }}>
          ...
          {recording ?
            <React.Fragment>
              <div className="serif" style={{ fontSize: 21, color: eDANGER, fontVariantNumeric: "tabular-nums" }}>{recTime}s</div>
              <button onClick={() => { setRecording(false); onChange(true); }} style={{ marginTop: 12, padding: "10px 22px", borderRadius: 10, background: "var(--danger-fill)", color: "#fff", border: "none", cursor: "pointer", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700 }}>Stop recording</button>
            </React.Fragment> :
            <React.Fragment>
              <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, marginBottom: 4 }}>Tap to record</div>
              <div style={{ fontFamily: "var(--sans)", fontSize: 15, color: eMUT }}>Max {q.maxDuration} seconds</div>
            </React.Fragment>
          }
        </div>)
      }
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 14px" }}>
              <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>Total</span>
              <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: ok ? eSUCCESS : eDANGER }}>{sum} / {q.target}</span>
            </div>
          </div>);
      })()}
      ...
      {q.type === "slider" && (() => {
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              {q.labels.map((l, i) => <span key={i} style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMUT }}>{l}</span>)}
            </div>
            {q.choices.map((c, i) =>
              <div key={i}>
                      ...
                      {g.cols.map((c, ci) => {
                        ...
                        return (
                          <button key={ci} onClick={() => onChange({ ...v, [si]: { ...(v[si] || {}), [gi]: ci } })} style={{ flex: "1 1 0", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "9px 10px", borderRadius: 9, border: "1.5px solid " + (sel ? eBLUE : eLINE), background: sel ? "color-mix(in srgb, var(--accent) 6%, transparent)" : "#fff", cursor: "pointer" }}>
                            <span style={{ width: 16, height: 16, flexShrink: 0, borderRadius: "50%", border: "2px solid " + (sel ? eBLUE : "var(--control-line)"), background: sel ? eBLUE : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>{sel && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--card)" }} />}</span>
                            <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: sel ? 700 : 700, color: sel ? eMID : eINK }}>{c}</span>
                          </button>);
                      })}
                    </div>
                  </div>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 480 }}>
              <thead>
                <tr>
                  <th></th>
                  {q.groups.map((g, gi) => <th key={gi} colSpan={g.cols.length} style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, padding: "6px 8px", textAlign: "center", borderBottom: "1px solid " + eLINE }}>{g.label}</th>)}
                </tr>
                <tr>
                  <th></th>
                  {q.groups.map((g, gi) => g.cols.map((c, ci) => <th key={gi + "-" + ci} style={{ fontFamily: "var(--sans)", fontSize: 15, color: eMUT, fontWeight: 700, padding: "4px 10px", textAlign: "center" }}>{c}</th>))}
                </tr>
              </thead>
              <tbody>
                {q.statements.map((s, si) =>
                      ...
                      {[0, 1, 2].map((wi) => {
                        ...
                        return (
                          <button key={wi} onClick={() => { const nw = [...why]; nw[wi] = !on; set(ci, { why: nw }); }} aria-label={cat + " — reason " + (wi + 1)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 11px", borderRadius: 8, border: "1.5px solid " + (on ? eBLUE : eLINE), background: on ? "color-mix(in srgb, var(--accent) 6%, transparent)" : "#fff", cursor: "pointer" }}>
                            ...
                            <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID }}>{wi + 1}</span>
                          </button>);
                      })}
                    </div>
                  </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {pool.map((it) => { const on = pickSel === it; return (
                  <button key={it} onClick={() => setPickSel(on ? null : it)} style={{ padding: "10px 13px", borderRadius: 2, border: (on ? "2px solid " + eBLUE : "1px dashed " + eLINE), background: on ? "color-mix(in srgb, var(--accent) 8%, transparent)" : eCARD, fontFamily: "var(--sans)", fontSize: 15, fontWeight: on ? 700 : 700, color: on ? eMID : eINK, cursor: "pointer" }}>{it}</button>); })}
              </div>
            </div>
    ...
    <div style={{ position: "fixed", right: 179, bottom: 14, zIndex: 60 }}>
      {layoutMenu &&
        <div style={{ position: "absolute", bottom: 42, right: 0, width: 256, background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, boxShadow: "0 10px 34px rgba(0,15,71,.16)", padding: 7, fontFamily: "var(--sans)" }}>
          <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: 0.2, color: "var(--muted)", padding: "7px 9px 5px" }}>Survey layout</div>
          ...
          {layout === "split" &&
            <React.Fragment>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: 0.2, color: "var(--muted)", padding: "7px 9px 5px" }}>Alignment</div>
