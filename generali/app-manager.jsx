...
const MgrTag = ({ kind }) => {
  ...
  const map = {
    ...
    New:    { c: "var(--accent)", b: "color-mix(in srgb, var(--accent) 15%, #ffffff)" },
  };
          ...
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            {groups[skill].map((c, i) => (
              <li key={i} style={{ fontFamily: "var(--sans)", fontSize: 13, color: eINK, lineHeight: 1.9 }}>
                ...
                <span style={{ background: c.kind === "added" ? "color-mix(in srgb, var(--accent) 15%, #ffffff)" : c.kind === "removed" ? "color-mix(in srgb, var(--danger) 15%, #ffffff)" : "color-mix(in srgb, #CB7E03 15%, #ffffff)", color: c.kind === "added" ? "var(--accent)" : c.kind === "removed" ? "var(--danger)" : "#CB7E03", padding: "1px 8px", borderRadius: 6 }}>{c.label}</span>
              </li>
            ))}
          </ul>
        </div>
